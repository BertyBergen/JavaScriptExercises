...
const cityInfo = (array, city) =>
  array.forEach((elements, index) => {
    return elements === city
      ? console.log(`${elements} is at the index ${index} in myCities array`)
      : "";
  });
cityInfo(myCities, "London");

myCities.forEach((city, index) => cityInfo(myCities, city));

console.log(myCities.map((value, keys) => value));
